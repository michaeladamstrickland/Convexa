...
class ProbateCourtTracker {
    ...
    async trackProbateFilings(counties) {
    ...
    async scrapeOdysseyPortal(page, courtSystem, county) {
        const cases = [];
        try {
    ...
    async enrichProbateCases(cases) {
        const enrichedCases = [];
        for (const probateCase of cases) {
            try {
                ...
                const properties = await this.findDeceasedProperties(probateCase.deceased_name, probateCase.case_details);
    ...
    async findDeceasedProperties(deceasedName, caseDetails) {
    ...
    async identifyHeirsFromCourt(probateCase) {
        const prompt = `
    ...
    async generateProbateDealBrief(probateCase, properties) {
    ...
    calculateProbateUrgency(probateCase, properties) {
        let urgencyScore = 0;
        ...
        if (daysSinceFiling <= 30)
            urgencyScore += 30; // Very fresh
        else if (daysSinceFiling <= 60)
            urgencyScore += 25;
            ...
            urgencyScore += 5; // Old cases, lower urgency
    ...
    calculateDealPotential(urgencyScore, properties) {
        const totalEquity = properties.reduce((sum, prop) => sum + (prop.equity || 0), 0);
        ...
        if (avgEquity >= 100000)
            dealScore += 40;
        else if (avgEquity >= 75000)
            dealScore += 30;
        else if (avgEquity >= 50000)
            dealScore += 20;
        else if (avgEquity >= 25000)
            dealScore += 10;
    ...
    identifyCourtSystem(county) {
        // Find matching court system or return default
        const system = this.courtSystems.find(cs => cs.county.toLowerCase() === county.toLowerCase());
        return system || {
            county: county,
    ...
    extractAttorneyInfo(probateCase) {
    ...
    isProbateCase(caseType) {
        const probateKeywords = ['probate', 'estate', 'will', 'inheritance', 'decedent', 'guardian'];
        return probateKeywords.some(keyword => caseType.toLowerCase().includes(keyword));
    }
    parseOdysseyCaseData(caseText) {
    ...
    determineApproachStrategy(heirs, attorney, urgencyScore) {
        if (attorney) {
