...
exports.ObituaryDeathMiner = void 0;
...
class ObituaryDeathMiner {
    ...
    async mineDeathIntelligence(markets) {
    ...
    async scrapeLegacyDotCom(markets) {
        ...
        try {
            for (const market of markets) {
                ...
                const marketObituaries = await page.evaluate(() => {
                    ...
                    return Array.from(obituaryElements).map((element) => {
    ...
    async scrapeLocalNewspapers(markets) {
    ...
    async scrapeNewspaperSite(siteUrl) {
        try {
    ...
    async scrapeFuneralHomes(markets) {
    ...
    async parseObituaryFeeds(markets) {
    ...
    consolidateDeathRecords(allRecords) {
    ...
    async enrichWithPropertyIntelligence(deaths) {
        ...
        return enrichedDeaths.filter(death => death.estate_value_estimate >= 100000 &&
            death.probate_filing_probability >= 0.3);
    }
    async identifyHeirs(obituaryText, survivors) {
    ...
    async findDeceasedProperties(name, lastKnownAddress) {
    ...
    async detectPostDeathVacancy(properties) {
    ...
    estimateEstateValue(properties) {
        return properties.reduce((total, property) => total + property.estimated_value, 0);
    }
    calculateProbateFilingProbability(death, estateValue) {
        let probability = 0;
        // Estate value impact
    ...
    async searchForNewspaperSites(query) {
    ...
    async findLocalFuneralHomes(market) {
    ...
    async scrapeFuneralHomeSite(siteUrl) {
        // Implementation for funeral home scraping
        return [];
    }
    async findObituaryFeeds(market) {
    ...
    async executeMining(markets = ['Chicago', 'Phoenix', 'Dallas']) {
...
exports.ObituaryDeathMiner = ObituaryDeathMiner;
// Direct execution disabled in ESM build to avoid require/import.meta guard complexity.
//# sourceMappingURL=obituaryDeathMiner.js.map