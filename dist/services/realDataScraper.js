...
export class RealDataScraper {
            ...
            for (const endpoint of endpoints) {
                try {
                    ...
                    if (response.data.features) {
                        // ArcGIS format
                        violations = response.data.features.map((f) => f.attributes);
                    }
                    else if (response.data.records) {
                        // Open data format
                        violations = response.data.records.map((r) => r.fields);
                    }
    ...
    async createProbateLead(caseDetails) {
        try {
    ...
    async createViolationLead(violation) {
        try {
    ...
    async createTaxDelinquencyLead(taxData) {
        try {
            return await this.db.createLead({
    ...
    async createForeclosureLead(foreclosureData) {
        try {
            return await this.db.createLead({
