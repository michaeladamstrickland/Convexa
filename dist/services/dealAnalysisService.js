...
async function getRegionFromZip(zipCode) {
    ...
    try {
        ...
        if (response.data && response.data.property) {
            comparables = response.data.property
                .filter((prop) => prop.sale && prop.sale.saleDate)
                .map((prop) => {
                // Calculate distance if we have lat/long
                let distance = 0;
                if (latitude && longitude && prop.location.latitude && prop.location.longitude) {
                    distance = calculateDistance(latitude, longitude, prop.location.latitude, prop.location.longitude);
                }
...
function calculateConfidenceScore(comparables) {
...
function generateAdjustmentExplanations(comparables) {
    const explanations = [];
    if (comparables.length === 0) {
