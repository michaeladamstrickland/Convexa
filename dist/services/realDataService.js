...
class RealDataService {
            ...
            if (county.toLowerCase() === 'maricopa') {
                ...
                for (const delinquency of delinquencies) {
                    ...
                    if (existingLead) {
                        ...
                        await this.db.updateLeadStatus(existingLead.id, 'qualified', `Tax delinquent: $${delinquency.amount_owed}`);
                    }
                    else {
                        // Create new lead
                        const lead = await this.db.createLead({
        ...
        for (const lead of leads) {
            try {
                ...
                if (contactInfo.phone || contactInfo.email) {
                    ...
                    const updatedLead = await this.db.updateLeadStatus(lead.id, 'contacted', `Contact info found: ${contactInfo.phone || 'No phone'}, ${contactInfo.email || 'No email'}`);
    ...
    // HELPER METHODS
    async findLeadByAddress(address) {
        try {
    ...
    calculateProbateMotivation(filing) {
        let score = 80; // Base probate motivation is high
        if (filing.out_of_state_heirs)
            score += 10;
        if (filing.multiple_heirs)
            score += 5;
        if (filing.estate_value > 500000)
            score += 5;
        return Math.min(score, 100);
    }
    calculateViolationMotivation(violation) {
        let score = 60; // Base violation motivation
        if (violation.severity > 80)
            score += 20;
        if (violation.repeat_offender)
            score += 15;
        if (violation.estimated_cost > 25000)
            score += 10;
        return Math.min(score, 100);
    }
    calculateTaxMotivation(delinquency) {
        let score = 70; // Base tax delinquency motivation
        if (delinquency.years_delinquent > 2)
            score += 15;
        if (delinquency.amount_owed > 15000)
            score += 10;
        if (delinquency.foreclosure_notice)
            score += 15;
        return Math.min(score, 100);
    }
    // SIMULATION METHODS (Replace with real data sources)
    async simulateProbateFilings() {
    ...
    async simulateTaxDelinquencies() {
        return [
            {
