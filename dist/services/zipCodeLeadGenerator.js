...
export class ZipCodeLeadGenerator {
     ...
     */
    async searchTargetArea(zipCodes) {
    ...
    async searchProbateByZip(zipCode) {
    ...
    async searchExpiredByZip(zipCode) {
    ...
    async searchViolationsByZip(zipCode) {
    ...
    async searchTaxDelinquentByZip(zipCode) {
    ...
    async searchHighEquityByZip(zipCode) {
    ...
    async searchAbsenteeByZip(zipCode) {
    ...
    generateViolationsForZip(zipCode) {
        const cityData = this.getZipCodeInfo(zipCode);
        const streetNames = this.getStreetNamesForZip(zipCode);
        const leads = [];
    ...
    generateTaxDelinquentForZip(zipCode) {
        const cityData = this.getZipCodeInfo(zipCode);
        const streetNames = this.getStreetNamesForZip(zipCode);
        const leads = [];
    ...
    generateHighEquityForZip(zipCode) {
        const cityData = this.getZipCodeInfo(zipCode);
        const streetNames = this.getStreetNamesForZip(zipCode);
        const leads = [];
    ...
    generateAbsenteeForZip(zipCode) {
        const cityData = this.getZipCodeInfo(zipCode);
        const streetNames = this.getStreetNamesForZip(zipCode);
        const leads = [];
    ...
    getStreetNamesForZip(zipCode) {
        ...
        if (zipCode.startsWith('852'))
            return scottsdaleStreets; // Scottsdale
        if (zipCode.startsWith('8528'))
            return tempeStreets; // Tempe
        return phoenixStreets; // Phoenix default
    }
    generatePropertyValue(zipCode) {
    ...
    async saveLeadToDatabase(lead) {
        try {
            ...
            if (lead.leadType === 'probate_intelligence')
                motivationScore += 10;
            if (lead.leadType === 'tax_delinquency')
                motivationScore += 15;
