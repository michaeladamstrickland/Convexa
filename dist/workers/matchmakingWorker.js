...
import { MATCHMAKING_QUEUE_NAME } from '../queues/matchmakingQueue';
...
export const matchmakingWorker = new Worker(MATCHMAKING_QUEUE_NAME, async (job) => {
    ...
    try {
        const mm = await prisma.matchmakingJob.findUnique({ where: { id: matchmakingJobId } });
        if (!mm)
            return;
        // Mark running
        await prisma.matchmakingJob.update({ where: { id: matchmakingJobId }, data: { status: 'running' } });
        incrStatus('running');
        ...
        const matchedCount = await prisma.scrapedProperty.count({ where });
        await prisma.matchmakingJob.update({ where: { id: matchmakingJobId }, data: { status: 'completed', matchedCount, completedAt: new Date() } });
        ...
        try {
            const activity = await prisma.crmActivity.create({ data: { type: 'matchmaking.completed', metadata: { matchmakingJobId, matchedCount } } });
        ...
        await prisma.matchmakingJob.update({ where: { id: job.data.matchmakingJobId }, data: { status: 'failed' } }).catch(() => { });
        incrStatus('failed');
...
export async function shutdownMatchmakingWorker() {
    ...
    try {
        await matchmakingWorker.connection?.disconnect?.();
    }
    catch { }
}
try {
    registerWorker(matchmakingWorker);
}
catch { }
