...
export class DealExecutionEngine {
    ...
    async executeDealCampaign(motivation) {
    ...
    determineCampaignStrategy(motivation) {
    ...
    async createPersonalizationProfile(motivation) {
        const prompt = `
    ...
    ${motivation.motivation_factors.map((factor, index) => `${index + 1}. ${factor.description} (Impact: ${factor.impact}/100, Urgency: ${factor.urgency})`).join('\\n')}
    ...
    createExecutionSchedule(strategy, motivation) {
        const schedule = [];
        ...
        for (let week = 0; week < totalWeeks; week++) {
            for (let touch = 0; touch < touchesPerWeek; touch++) {
                if (stepNumber === 1)
                    continue; // Skip first contact already added
    ...
    async launchImmediateCampaign(campaign, motivation) {
    ...
    async scheduleDelayedCampaign(campaign, motivation) {
    ...
    async executeColdCall(campaign, motivation) {
    ...
    async generateCallScript(campaign, motivation) {
    ...
    async executeSMSSequence(campaign, motivation) {
    ...
    async generateSMSSequence(campaign, motivation) {
        const personalization = campaign.personalization_data;
        const prompt = `
    ...
    async executeEmailSequence(campaign, motivation) {
    ...
    async executeDirectMail(campaign, motivation) {
    ...
    // Helper methods
    calculateOptimalCallTime(leadId) {
    ...
    async makeIntelligentCall(callData) {
    ...
    scheduleSMS(leadId, message, sendTime) {
        console.log(`📅 Scheduling SMS for ${leadId} at ${sendTime}`);
    }
    scheduleEmail(leadId, email, sendTime) {
        console.log(`📅 Scheduling email for ${leadId} at ${sendTime}`);
    }
    updateCampaignMetrics(campaign, action, result) {
    ...
    initializeMetricsTracking(leadId) {
        return {
            contact_attempts: 0,
    ...
    getDefaultPersonalizationProfile(motivation) {
        return {
    ...
    getDefaultCallScript(personalization) {
        return {
    ...
    getDefaultSMSSequence(personalization) {
        return {
            messages: [
                {
                    delay_hours: 0,
    ...
    async generateEmailSequence(campaign, motivation) {
    ...
    async generateDirectMailPiece(campaign, motivation) {
    ...
    async getCampaignStatus(leadId) {
        return this.activeExecutions.get(leadId) || null;
    }
    async updateCampaignResult(leadId, stepNumber, result) {
