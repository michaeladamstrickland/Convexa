...
export class AutomatedLeadScoring {
    ...
    async scoreAndQualifyLead(leadProfile) {
    ...
    calculateDistressSignals(leadProfile) {
        const signals = {
            tax_delinquency: 0,
        ...
        if (leadProfile.tax_debt > 0) {
            if (leadProfile.tax_debt > 25000)
                signals.tax_delinquency = 100;
            else if (leadProfile.tax_debt > 15000)
                signals.tax_delinquency = 80;
            else if (leadProfile.tax_debt > 10000)
                signals.tax_delinquency = 60;
            else if (leadProfile.tax_debt > 5000)
                signals.tax_delinquency = 40;
            else
                signals.tax_delinquency = 20;
        }
        // Foreclosure status
        if (leadProfile.foreclosure_stage) {
            switch (leadProfile.foreclosure_stage.toLowerCase()) {
                case 'auction_scheduled':
                    signals.foreclosure_status = 100;
                    break;
                case 'notice_of_default':
                    signals.foreclosure_status = 80;
                    break;
                case 'lis_pendens':
                    signals.foreclosure_status = 60;
                    break;
                case 'pre_foreclosure':
                    signals.foreclosure_status = 40;
                    break;
        ...
        if (leadProfile.vacancy_months) {
            if (leadProfile.vacancy_months >= 12)
                signals.vacancy_duration = 100;
            else if (leadProfile.vacancy_months >= 6)
                signals.vacancy_duration = 80;
            else if (leadProfile.vacancy_months >= 3)
                signals.vacancy_duration = 60;
            else
                signals.vacancy_duration = 40;
        }
        ...
        if (leadProfile.days_on_market) {
            if (leadProfile.days_on_market > 180)
                signals.days_on_market = 90;
            else if (leadProfile.days_on_market > 120)
                signals.days_on_market = 70;
            else if (leadProfile.days_on_market > 90)
                signals.days_on_market = 50;
            else if (leadProfile.days_on_market > 60)
                signals.days_on_market = 30;
        }
        ...
        if (leadProfile.owner_distance_miles) {
            if (leadProfile.owner_distance_miles > 1000)
                signals.out_of_state_distance = 80;
            else if (leadProfile.owner_distance_miles > 500)
                signals.out_of_state_distance = 60;
            else if (leadProfile.owner_distance_miles > 200)
                signals.out_of_state_distance = 40;
            else
                signals.out_of_state_distance = 20;
        }
        return signals;
    }
    async generateMotivationAnalysis(leadProfile, distressSignals) {
        const prompt = `
    ...
    calculateDealMetrics(leadProfile, motivation) {
        ...
        let repairCosts = 0;
        if (leadProfile.condition_score < 30)
            repairCosts = estimatedValue * 0.25;
        else if (leadProfile.condition_score < 50)
            repairCosts = estimatedValue * 0.15;
        else if (leadProfile.condition_score < 70)
            repairCosts = estimatedValue * 0.08;
        else
            repairCosts = estimatedValue * 0.03;
    ...
    calculateRiskScore(leadProfile, motivation) {
        let riskScore = 0;
        // Title/legal risks
        if (leadProfile.is_probate)
            riskScore += 20;
        if (leadProfile.is_divorce)
            riskScore += 15;
        if (leadProfile.foreclosure_stage)
            riskScore += 25;
        // Property condition risks
        if (leadProfile.condition_score < 40)
            riskScore += 30;
        else if (leadProfile.condition_score < 60)
            riskScore += 20;
        else if (leadProfile.condition_score < 80)
            riskScore += 10;
        ...
        // Market timing risks
        if (leadProfile.days_on_market && leadProfile.days_on_market > 120)
            riskScore += 15;
        // Motivation volatility
        if (motivation.overall_motivation_score < 50)
            riskScore += 20;
        return Math.min(100, riskScore);
    }
    estimateTimeToClose(motivation) {
        // Base timeline of 30 days
        let timeToClose = 30;
        // Adjust based on motivation and urgency
        if (motivation.overall_motivation_score >= 90)
            timeToClose = 7; // Highly motivated
        else if (motivation.overall_motivation_score >= 80)
            timeToClose = 14; // Very motivated
        else if (motivation.overall_motivation_score >= 70)
            timeToClose = 21; // Motivated
        else if (motivation.overall_motivation_score >= 60)
            timeToClose = 30; // Somewhat motivated
        else
            timeToClose = 45; // Lower motivation needs more time
        // Adjust for urgency timeline
        if (motivation.urgency_timeline <= 7)
            timeToClose = Math.min(timeToClose, 10);
        else if (motivation.urgency_timeline <= 30)
            timeToClose = Math.min(timeToClose, 21);
        return timeToClose;
    }
    calculateCompetitionLevel(leadProfile) {
        let competitionLevel = 5; // Base level
        // Attractive properties get more competition
        if (leadProfile.condition_score > 80)
            competitionLevel += 3;
        if (leadProfile.estimated_value > 300000)
            competitionLevel += 2;
        // Distressed properties have less competition
        if (leadProfile.violations.length > 3)
            competitionLevel -= 2;
        if (leadProfile.condition_score < 50)
            competitionLevel -= 3;
        if (leadProfile.tax_debt > 15000)
            competitionLevel -= 2;
        // Market factors
        if (leadProfile.days_on_market && leadProfile.days_on_market > 90)
            competitionLevel -= 1;
        if (leadProfile.price_reduction_count && leadProfile.price_reduction_count > 1)
            competitionLevel -= 1;
        return Math.max(1, Math.min(10, competitionLevel));
    }
    determineExecutionStatus(motivation, dealMetrics) {
    ...
    async generateActionPlan(leadProfile, motivation, dealMetrics) {
    ...
    calculateFallbackMotivationScore(distressSignals) {
        const weights = {
    ...
    getDefaultMotivationFactors(distressSignals) {
        const factors = [];
    ...
    calculateUrgencyTimeline(distressSignals) {
        if (distressSignals.foreclosure_status > 80)
            return 7; // Immediate
        if (distressSignals.foreclosure_status > 60)
            return 30; // 1 month
        if (distressSignals.tax_delinquency > 80)
            return 60; // 2 months
        if (distressSignals.divorce_proceedings > 0)
            return 90; // 3 months
        return 180; // Default 6 months
    }
    calculatePredictedDiscount(distressSignals) {
        let discount = 5; // Base 5% discount
        if (distressSignals.foreclosure_status > 80)
            discount += 20;
        else if (distressSignals.foreclosure_status > 60)
            discount += 15;
        else if (distressSignals.foreclosure_status > 40)
            discount += 10;
        if (distressSignals.tax_delinquency > 60)
            discount += 8;
        if (distressSignals.condition_score > 70)
            discount += 12;
        if (distressSignals.vacancy_duration > 60)
            discount += 10;
        return Math.min(35, discount); // Cap at 35% discount
    }
    calculateDealProbability(distressSignals) {
        let probability = 15; // Base 15%
        if (distressSignals.foreclosure_status > 60)
            probability += 30;
        if (distressSignals.tax_delinquency > 70)
            probability += 25;
        if (distressSignals.probate_status > 0)
            probability += 20;
        if (distressSignals.divorce_proceedings > 0)
            probability += 20;
        if (distressSignals.vacancy_duration > 60)
    ...
    getDefaultTalkingPoints(leadProfile) {
        const points = [
    ...
    generateFallbackMotivationAnalysis(leadProfile, distressSignals) {
        return {
            property_id: leadProfile.property_id,
