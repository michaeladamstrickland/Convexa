...
router.get('/revenue-analytics', async (req, res) => {
  try {
    ...
    const analytics = {
      ...
      hotLeads: leads.filter(lead => (lead.motivation_score || 0) >= 75).length,
      warmLeads: leads.filter(lead => 
                (lead.motivation_score || 0) >= 50 && (lead.motivation_score || 0) < 75).length,
      probateLeads: leads.filter(lead => lead.is_probate).length,
      vacantLeads: leads.filter(lead => lead.is_vacant).length,
