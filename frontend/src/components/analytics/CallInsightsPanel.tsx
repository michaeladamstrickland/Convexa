...
export default function CallInsightsPanel() {
  ...
  if (q.isFetching) {
    // lightweight runtime heartbeat for diagnostics
    // eslint-disable-next-line no-console
    console.debug("[CallInsightsPanel] refetching /api/dev/metrics at", new Date().toISOString());
  }

  const raw = q.data ?? "";
  const metrics = useMemo(() => (raw ? parsePrometheusText(raw) : ({} as any)), [raw]);
  ...
  const prometheusLink = useMemo(() => {
    const base = "/api/dev/metrics";
    const params = new URLSearchParams();
    Object.entries(labelFilters).forEach(([k, v]) => params.append(k, v));
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {q.isLoading ? (
        <Card>Loading call insights…</Card>
      ) : q.error ? (
        <Card error>Failed to load call metrics</Card>
      ) : (
        <>
          <Card>
...
function ExportButton({ raw, metrics, labelFilters }: { raw: string; metrics: any; labelFilters: Record<string, string> }) {
  ...
  <button className="text-xs px-2 py-1 rounded border hover:bg-gray-50 dark:hover:bg-gray-800" onClick={handleExportJSON}>Export JSON</button>
  <button className="text-xs px-2 py-1 rounded border hover:bg-gray-50 dark:hover:bg-gray-800" onClick={handleExportCSV}>Export CSV</button>
    </div>
  );
}
