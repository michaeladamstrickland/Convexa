...
interface MotivationWeights {
  financial_distress: number;
  property_distress: number;
  personal_distress: number;
  market_distress: number;
  logistical_distress: number;
}

interface MotivationNeuralNetwork {
...
export class MotivationPredictor {
  ...
  private async calculateMotivationScore(signals: DistressSignals): Promise<any> {
    ...
    const overallScore = 
      (financialScore * this.neuralNetwork.weights.financial_distress) +
  ...
  private calculateFinancialDistress(signals: DistressSignals): number {
    let score = 0;
  ...
  private calculatePropertyDistress(signals: DistressSignals): number {
    let score = 0;
  ...
  private calculatePersonalDistress(signals: DistressSignals): number {
    let score = 0;
  ...
  private calculateMarketDistress(signals: DistressSignals): number {
    let score = 0;
  ...
  private calculateLogisticalDistress(signals: DistressSignals): number {
    let score = 0;
  ...
  private identifyContributingFactors(scores: any, signals: DistressSignals): MotivationFactor[] {
  ...
  private async generateMotivationInsights(profile: LeadProfile, score: any): Promise<any> {
    ...
    ${score.contributing_factors.map((factor: MotivationFactor, index: number) => 
  ...
  private getDefaultInsights(motivationScore: number): any {
    return {
  ...
  private quantifyForeclosureStage(stage?: string): number {
    if (!stage) return 0;
  ...
  private calculateConfidenceLevel(signals: DistressSignals): number {
  ...
  private initializeNeuralNetwork(): MotivationNeuralNetwork {
  ...
  async updateNeuralNetwork(outcomes: any[]): Promise<void> {
  ...
  private getMotivationRange(score: number): string {
    if (score >= this.neuralNetwork.thresholds.high_motivation) return 'high';
    if (score >= this.neuralNetwork.thresholds.medium_motivation) return 'medium';
    return 'low';
  }

  // Batch prediction for multiple leads
  async predictBatchMotivation(leadProfiles: LeadProfile[]): Promise<MotivationIntelligence[]> {
    console.log(`🧠 Batch predicting motivation for ${leadProfiles.length} leads...`);
    
    const predictions = await Promise.all(
      leadProfiles.map(profile => this.predictMotivation(profile))
    );
