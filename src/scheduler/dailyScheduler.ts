...
const ranSourcesToday = new Set<string>();

function todayKey(): string { return new Date().toISOString().slice(0,10); }

function parseTargetTime(): { hh: number; mm: number } {
  const raw = process.env.SCHEDULER_DAILY_HHMM || '06:30';
  const m = raw.match(/^(\d{2}):(\d{2})$/);
  if (!m) return { hh: 6, mm: 30 };
  return { hh: parseInt(m[1], 10), mm: parseInt(m[2], 10) };
}

async function alreadyEnqueuedToday(source: string): Promise<boolean> {
  const start = new Date();
  start.setHours(0,0,0,0);
  const existing = await prisma.scraperJob.findFirst({
    where: { source: source as any, createdAt: { gte: start } },
    select: { id: true }
  });
  return !!existing;
}
...
export async function triggerDailyScheduler(force = false) {
  ...
  const sources: Array<'zillow'|'auction'> = ['zillow','auction'];
  const enqueued: Array<{ source: string; jobId?: string; skipped: boolean; reason?: string }> = [];
  for (const source of sources) {
    const skipReason: string[] = [];
    let skipped = false;
    if (!force) {
      if (ranSourcesToday.has(source)) { skipped = true; skipReason.push('already_ran_in_memory'); }
      else if (await alreadyEnqueuedToday(source)) { skipped = true; skipReason.push('already_enqueued_today'); }
    } else if (ranSourcesToday.has(source)) {
      // even on force, prevent true duplicate inside same manual call cycle
      skipped = true; skipReason.push('duplicate_in_request');
    }
    if (skipped) {
...
export function startDailyScheduler() {
  ...
  setInterval(async () => {
    try {
      ...
      if (ranSourcesToday.size >= 2) return; // all sources done
