...
interface ROIScenario extends ROICalculation {
  ...
  sensitivityAnalysis: {
    ...
    timeToSell: SensitivityRange; // months
  };
}
 ...
 */
export function calculateFinancingCosts(
...
export function createROIScenarios(
  ...
  const cashScenario: ROIScenario = {
  ...
  cashScenario.timelineImpact = {
    bestCase: calculateROIPercent(
      cashScenario.sensitivityAnalysis.purchasePrice.min,
      cashScenario.sensitivityAnalysis.renovationCost.min,
      cashScenario.sensitivityAnalysis.projectedARV.max,
      cashScenario.sensitivityAnalysis.timeToSell.min
    ),
    expected: cashScenario.roi,
    worstCase: calculateROIPercent(
      cashScenario.sensitivityAnalysis.purchasePrice.max,
      cashScenario.sensitivityAnalysis.renovationCost.max,
      cashScenario.sensitivityAnalysis.projectedARV.min,
      cashScenario.sensitivityAnalysis.timeToSell.max
    )
  };
...
export function estimateARV(
  ...
  if (kitchen) arvMultiplier += 0.05;
  if (bathrooms) arvMultiplier += 0.05;
  if (flooring) arvMultiplier += 0.03;
  if (paint) arvMultiplier += 0.02;
  if (exterior) arvMultiplier += 0.04;
  if (landscaping) arvMultiplier += 0.02;
  if (structural) arvMultiplier += 0.05;
