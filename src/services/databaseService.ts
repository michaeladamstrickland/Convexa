...
import { LeadProfile, ProbateCase, PropertyViolation, LeadSourceType } from '../types';
...
export class DatabaseService {
  ...
  // Lead Management
  async createLead(leadData: CreateLeadData): Promise<any> {
  ...
  async getLeads(filters?: any): Promise<any[]> {
    ...
    if (filters?.is_probate !== undefined) where.is_probate = filters.is_probate;
  ...
  async getHighValueLeads(limit: number = 50): Promise<any[]> {
    return await this.prisma.lead.findMany({
      where: {
        lead_score: { gte: 70 }
      },
  ...
  async getLeadByAddress(address: string): Promise<any | null> {
  ...
  async createProbateCase(caseData: Partial<ProbateCase>): Promise<any> {
  ...
  async getProbateCases(filters?: any): Promise<any[]> {
    const where: any = {};
    
    if (filters?.county) where.county = filters.county;
    if (filters?.status) where.case_status = filters.status;
    if (filters?.minValue) where.estimated_estate_value = { gte: filters.minValue };
  ...
  async createViolation(violationData: Partial<PropertyViolation>): Promise<any> {
  ...
  async getViolations(filters?: any): Promise<any[]> {
    ...
    if (filters?.type) where.violation_type = filters.type;
    if (filters?.minSeverity) where.severity_score = { gte: filters.minSeverity };
    if (filters?.stage) where.enforcement_stage = filters.stage;
  ...
  // Utility Methods
  private calculateBasicLeadScore(leadData: CreateLeadData): number {
    let score = 0;
    ...
    if (equity > 200000) score += 20;
    else if (equity > 100000) score += 15;
    else if (equity > 50000) score += 10;

    // Distress indicators
    if (leadData.is_probate) score += 25;
    if (leadData.tax_debt && leadData.tax_debt > 10000) score += 15;
    if (leadData.violations && leadData.violations > 0) score += 10;

    // Source type bonus
    if (leadData.source_type === 'probate_intelligence') score += 15;
    if (leadData.source_type === 'tax_delinquency') score += 20;
    if (leadData.source_type === 'pre_foreclosure') score += 25;

    return Math.min(Math.max(score, 0), 100);
  }

  private calculateLeadScore(leadData: Partial<LeadProfile>): number {
    let score = 0;
    ...
    if (leadData.is_probate) score += 25;
    if (leadData.tax_debt && leadData.tax_debt > 10000) score += 15;
    if (leadData.violations && leadData.violations.length > 0) score += 10;
    if (leadData.vacancy_months && leadData.vacancy_months > 3) score += 10;

    // Condition penalty
    const condition = leadData.condition_score || 50;
    if (condition < 30) score += 15; // Very distressed = opportunity
    else if (condition > 80) score -= 5; // Too good condition
